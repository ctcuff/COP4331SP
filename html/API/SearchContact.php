<?php
   ...
   $conn = new mysqli($servername, $username, $password);
	...
	else
	{
      ...
      // value used to represent a query to return all contacts the user has
      $SHOW_ALL_QUERY = "";

      if($inData["Query"] == $SHOW_ALL_QUERY)
      {
         $sql = $conn->prepare("SELECT Contact_ID,First_Name,Last_Name,Phone,Email FROM CONTACTS WHERE User_ID=?");
         $sql->bind_param("i", $inData["User_ID"]);
      }
      else
      {
         // note that we use a partial match here
         // so that the user doesnt have to be exact.
